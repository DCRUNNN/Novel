...
var NovelItem = function (text) {
    if (text) {
        ...
        this.numOfAgree = obj.numOfAgree; // 小说被点赞的数量
        this.numOfDiss = obj.numOfDiss; // 小说被踩的数量
...
NovelService.prototype = {
    ...
    createNovel: function(title,subject,content) {
        ...
        novelItem.time = time;
        novelItem.numOfAgree = 0;
        novelItem.numOfDiss = 0;
        novelItem.agreedUsers = [];
        novelItem.disagreedUsers = [];
    ...
    // 更新现有小说
    updateNovel: function(subject, content) {
        var from = Blockchain.transaction.from;
        var time = Blockchain.block.timestamp;
        if(subject=="" || content=="" ) {
        ...
        var isExisted = 0; //0代表未出现，1代表出现
        for(var i=0;i<novelItem.authors.length;i++){
            if(novelItem.authors[i]==from){
                isExisted=1;
            }
        }
        if(isExisted==0){
    ...
    getNovelBySubject: function (subject) {
        return this.novelRepo.get(subject);
    },

    voteToNovel: function (subject, opinion) {
        ...
        if (opinion=="agree") {
            novelItem.numOfAgree += 1;
            novelItem.agreedUsers.push(from);
        } else {
            novelItem.numOfDiss += 1;
